...
from utils.infinity_rope_integration import (
    apply_block_relativistic_rope,
    attach_infinity_rope_adapter,
    block_relative_frame_indices,
    cache_key_for_storage,
    infinity_rope_enabled,
)
from utils.deep_forcing import (
    attach_deep_forcing_adapter,
    deep_forcing_enabled,
...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            use_deep_forcing = deep_forcing_enabled(self)
            if use_infinity_rope and use_deep_forcing:
                raise ValueError(
                    "Deep Forcing and Infinity-RoPE cannot be enabled together"
                )
            legacy_roped_key = (
                k
                if use_infinity_rope
                else causal_rope_apply(
                    k, grid_sizes, freqs, start_frame=current_start_frame
                ).type_as(v)
            )
            cache_key = cache_key_for_storage(
                self,
                raw_key=k,
                legacy_roped_key=legacy_roped_key,
            )
            ...
            precomputed_roped_query = None
            if use_deep_forcing:
                precomputed_roped_query = causal_rope_apply(
                    q,
                    grid_sizes,
                    freqs,
                    start_frame=current_start_frame,
                ).type_as(v)
                cache_update_info = prepare_deep_forcing_cache(
                    self,
                    kv_cache,
                    roped_query=precomputed_roped_query,
                    roped_key=cache_key,
                    value=v,
                    freqs=freqs,
                    ...
                    max_attention_tokens=self.max_attention_size,
                )
                local_start_index = cache_update_info["local_start_index"]
                local_end_index = cache_update_info["local_end_index"]
                temp_k = cache_update_info["new_k"]
                temp_v = cache_update_info["new_v"]
                ...
                if write_len > 0:
                    temp_k[:, write_start_index:local_end_index] = cache_key[:, roped_offset:roped_offset + write_len]
                ...
                cache_update_info = {
                    ...
                    "new_k": cache_key[:, roped_offset:roped_offset + write_len],
            ...
            if use_infinity_rope:
                ...
                cache_capacity_frames = kv_cache_size // frame_seqlen
                query_relative_indices, cache_relative_indices = (
                    block_relative_frame_indices(
                        current_start_frame=int(current_start_frame),
                        num_query_frames=num_new_frames,
                        resident_cache_frames=num_cache_frames,
                        cache_capacity_frames=cache_capacity_frames,
                        device=k.device,
                    )
                )
                cache_grid_sizes = grid_sizes.clone()
                cache_grid_sizes[:, 0] = num_cache_frames
                attention_k = apply_block_relativistic_rope(
                    attention_k,
                    cache_grid_sizes,
                    freqs,
            ...
            if sink_tokens > 0:
                ...
                if local_budget > 0:
                    ...
                    k_cat = k_sink
                    v_cat = v_sink
                if precomputed_roped_query is not None:
                    roped_query = precomputed_roped_query
                else:
                    roped_query = (
                        apply_block_relativistic_rope(
                ...
                if bool(
                    ...
                    source_frames = build_temporal_key_source_frames(
                        ...
                        warmup_tokens=0,
                        memory_tokens=0,
                        local_tokens=int(k_cat.shape[1]) - sink_tokens,
                        current_start_frame=int(current_start_frame),
                        num_query_frames=int(grid_sizes[0][0].item()),
                ...
                x = attention(
                    roped_query,
                    attention_k[:, window_start:local_end_index],
    ...
    def _apply_cache_updates(self, kv_cache, cache_update_infos):
        ...
        for block_index, (current_end, local_end_index, update_info) in cache_update_infos:
            if update_info is not None:
                ...
                if update_info["action"] == "deep_forcing_replace":
                    cache["k"].copy_(update_info["new_k"])
                    cache["v"].copy_(update_info["new_v"])
                    for state_name in (
                        "topc_select_counts",
                        "abs_frame_idx",
                        "protected_mask",
                    ):
                        state_value = update_info[state_name]
                        if state_name in cache and cache[state_name].shape == state_value.shape:
                            cache[state_name].copy_(state_value)
                        else:
                            cache[state_name] = state_value.clone()
                    cache["protected_len"] = update_info["protected_len"].clone()
                    cache["protected_len_max"] = cache["protected_len"].max()
                    for state_name in (
                        "sink_base_abs_start_frame",
                        "topc_base_abs_start_frame",
                    ):
                        state_value = update_info.get(state_name)
                        if state_value is None:
                            continue
                        if state_name in cache:
                            cache[state_name].fill_(int(state_value))
                        else:
                            cache[state_name] = torch.tensor(
                                int(state_value),
                                dtype=torch.long,
