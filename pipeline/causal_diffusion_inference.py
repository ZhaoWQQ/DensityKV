...
class CausalDiffusionInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        for integration_key in (
            "density_kv",
            "temporal_attention_trace",
        ):
            model_kwargs.pop(integration_key, None)
        ...
        self.sample_solver = str(getattr(args, "sample_solver", "unipc"))
    ...
    def inference(
        ...
        self.frame_seq_length = (height // patch_size[1]) * (width // patch_size[2])
