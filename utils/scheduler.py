...
class FlowMatchScheduler():
    ...
    def step_to(self, model_output, timestep, sample, next_timestep=None):
        """Euler-integrate a flow prediction to an explicit next timestep."""
        if timestep.ndim == 2:
            timestep = timestep.flatten(0, 1)
        self.sigmas = self.sigmas.to(model_output.device)
        self.timesteps = self.timesteps.to(model_output.device)
        timestep = timestep.to(device=model_output.device)
        timestep_id = torch.argmin(
        ...
        if next_timestep is None:
            next_sigma = torch.zeros_like(sigma)
        else:
            if not torch.is_tensor(next_timestep):
                next_timestep = timestep.new_full(timestep.shape, next_timestep)
            else:
                next_timestep = next_timestep.to(device=model_output.device)
                if next_timestep.ndim == 0:
                    next_timestep = next_timestep.expand_as(timestep)
                elif next_timestep.ndim == 2:
                    next_timestep = next_timestep.flatten(0, 1)
                if next_timestep.numel() == 1 and timestep.numel() != 1:
                    next_timestep = next_timestep.expand_as(timestep)
            next_id = torch.argmin(
                (self.timesteps.unsqueeze(0) - next_timestep.unsqueeze(1)).abs(),
                dim=1,
            )
            next_sigma = self.sigmas[next_id]
            next_sigma = torch.where(
                next_timestep <= 0,
                torch.zeros_like(next_sigma),
                next_sigma,
            )

        dimensions = (1,) * (sample.ndim - 1)
        sigma = sigma.reshape(-1, *dimensions)
        next_sigma = next_sigma.reshape(-1, *dimensions)
        result = (
            sample.float()
            + model_output.float() * (next_sigma.float() - sigma.float())
        )
