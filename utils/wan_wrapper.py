...
from wan.modules.causal_model_latentmem import CausalWanModel as CausalWanModelLatentMem
from utils.deep_forcing import parse_deep_forcing_config
...
class WanVAEWrapper(torch.nn.Module):
    ...
    def decode_to_pixel_chunk(self, latent: torch.Tensor, use_cache: bool = False, chunk_size: int = 60) -> torch.Tensor:
        """
        ...
        Args:
            ...
            chunk_size: Number of latent frames to decode at once (default 60 to avoid OOM)
        ...
        output = []
        for u in zs:
            # cached_decode carries causal temporal features across chunks. Reset
            # only at video boundaries so each chunk continues the same video.
    ...
    def begin_stream_decode(self) -> None:
        """Reset causal VAE state at the start of one video."""
        self.model.clear_cache()

    @torch.no_grad()
    def decode_to_pixel_stream_chunk(self, latent: torch.Tensor) -> torch.Tensor:
        """Decode one latent shard while preserving VAE state for the next shard."""
        if latent.ndim != 5 or latent.shape[0] != 1:
            raise ValueError("streaming VAE decode requires latent shape [1, T, C, H, W]")
        zs = latent.permute(0, 2, 1, 3, 4)
        device, dtype = latent.device, latent.dtype
        scale = [
        ...
        decoded = self.model.cached_decode(zs[0].unsqueeze(0), scale)
        decoded = decoded.float().clamp_(-1, 1).squeeze(0)
        return decoded.permute(1, 0, 2, 3).unsqueeze(0).cpu()

    def end_stream_decode(self) -> None:
        """Release causal VAE state after the final shard."""
        self.model.clear_cache()
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            deep_forcing_config = parse_deep_forcing_config(deep_forcing)
            if use_latentmem and deep_forcing_config.enabled:
                raise ValueError(
                    "Deep Forcing must use the native roped-K cache path; "
                    "use_latentmem=true is unsupported"
                )
            if use_latentmem:
                self.model = CausalWanModelLatentMem.from_pretrained(
                    f"wan_models/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size,
                    memory_size=memory_size, infinity_rope=infinity_rope)
            else:
                self.model = CausalWanModel.from_pretrained(
                    f"wan_models/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size,
                    infinity_rope=infinity_rope, deep_forcing=deep_forcing)
        else:
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            if memory_indices is not None:
                fwd_kwargs["memory_indices"] = memory_indices
            ...
            if clean_x is not None:
                ...
                if classify_mode:
                    ...
                    if not self.uniform_timestep:
                        flow_pred_kwargs["sink_recache_after_switch"] = sink_recache_after_switch
